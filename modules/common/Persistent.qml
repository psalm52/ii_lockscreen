...
Singleton {
    ...
    // Standalone build note :: original tracked HYPRLAND_INSTANCE_SIGNATURE to detect
    // "fresh compositor login" for the lock-on-startup feature. Not used in this build
    // (locking happens only via keybind/idle), so this is simplified to a static false.
    property bool isNewHyprlandInstance: false
