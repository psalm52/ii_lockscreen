...
readonly property string monthNumber: TimeManager._pad(TimeManager.month)
    ...
    readonly property bool sessionLockedActive: {
        return Settings.lock_showLockedText && Settings.sessionLockedText !== ""; // standalone build :: config.* was SDDM-only
    }
    readonly property bool quoteActive: {
        return Settings.background_widgets_clock_quote_enable && Settings.background_widgets_clock_quote_text !== ""; // standalone build :: config.* was SDDM-only
    }
    ...
    ColumnLayout {
        ...
        Loader {
            ...
            Layout.alignment: Qt.AlignHCenter
            onLoaded: {
                item.text = Settings.sessionLockedText;
                item.backgroundColor = Colors.secondary_container;
                item.textColor = Colors.on_secondary_container;
                item.shadowColor = Colors.colShadow;
            }
