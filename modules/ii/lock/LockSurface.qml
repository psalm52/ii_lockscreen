...
import Quickshell
// Standalone build note :: dropped `import qs.modules.ii.bar as Bar` and
// `Quickshell.Services.SystemTray` -- both were only used for the optional Fcitx
// tray-icon indicator (Bar.SysTray below), which pulled in a Hyprland-specific
// HyprlandFocusGrab dependency via SysTray.qml. Not needed for the lock itself.
// Standalone build addition :: CookieClock ported from psalm52/illogical_sddm
// (itself adapted from end-4's dots). Namespaced import -- the clock subtree
// carries its own Colors/Settings/Appearance/TimeManager singletons, which must
// not collide with qs.modules.common's Appearance already imported above.
import "clock" as IIClock
...
MouseArea {
    ...
    Toolbar {
        ...
        // read HyprlandXkb.currentLayoutCode -- Hyprland-only IPC data with no generic
        // Wayland equivalent exposed to Quickshell. Also dropped the Fcitx tray-icon
        // block (Bar.SysTray), which existed only to show an IME indicator and pulled
        // in a Hyprland-specific focus-grab dependency. Neither is required for locking;
        // add a generic replacement here if you want a layout indicator back.
    }
